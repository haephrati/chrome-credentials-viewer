...
BEGIN_MESSAGE_MAP(SG_Button, CButton)
    //{{AFX_MSG_MAP(SG_Button)
	ON_WM_SETCURSOR()
	ON_WM_KILLFOCUS()
	ON_WM_MOUSEMOVE()
...
void SG_Button::FreeResources(BOOL bCheckForNULL)
{
	if (bCheckForNULL)
	{
		// Destroy icons
...
void SG_Button::PreSubclassWindow() 
{
	UINT nBS;
...
BOOL SG_Button::PreTranslateMessage(MSG* pMsg) 
{
	InitToolTip();
...
LRESULT SG_Button::DefWindowProc(UINT message, WPARAM wParam, LPARAM lParam) 
{
...
HBRUSH SG_Button::CtlColor(CDC* pDC, UINT nCtlColor) 
{
	return (HBRUSH)::GetStockObject(NULL_BRUSH); 
} // End of CtlColor

void SG_Button::OnSysColorChange() 
{
...
LRESULT SG_Button::OnSetStyle(WPARAM wParam, LPARAM lParam)
{
...
LRESULT SG_Button::OnSetCheck(WPARAM wParam, LPARAM lParam)
{
...
LRESULT SG_Button::OnGetCheck(WPARAM wParam, LPARAM lParam)
{
	ASSERT(m_bIsCheckBox);
	return GetCheck();
} // End of OnGetCheck
...
#ifdef	ST_BTN_USE_BCMENU
LRESULT SG_Button::OnMenuChar(UINT nChar, UINT nFlags, CMenu* pMenu) 
{
	LRESULT lResult;
...
#ifdef	ST_BTN_USE_BCMENU
void SG_Button::OnMeasureItem(int nIDCtl, LPMEASUREITEMSTRUCT lpMeasureItemStruct) 
{
	BOOL bSetFlag = FALSE;
...
void SG_Button::OnEnable(BOOL bEnable) 
{
...
void SG_Button::OnKillFocus(CWnd * pNewWnd)
{
	CButton::OnKillFocus(pNewWnd);
	CancelHover();
} // End of OnKillFocus

void SG_Button::OnActivate(UINT nState, CWnd* pWndOther, BOOL bMinimized) 
{
	CButton::OnActivate(nState, pWndOther, bMinimized);
	if (nState == WA_INACTIVE)	CancelHover();
} // End of OnActivate

void SG_Button::OnCancelMode() 
{
	CButton::OnCancelMode();
	CancelHover();
} // End of OnCancelMode

BOOL SG_Button::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) 
{
...
void SG_Button::OnMouseMove(UINT nFlags, CPoint point)
{
...
LRESULT SG_Button::OnMouseLeave(WPARAM wParam, LPARAM lParam)
{
	CancelHover();
	return 0;
} // End of OnMouseLeave

BOOL SG_Button::OnClicked() 
{	
	SetFocus();
...
void SG_Button::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
	...
	if (sTitle.IsEmpty() == FALSE)
	{
		...
		*/
		...
		else
		{
			...
			{
				pDC->SetTextColor(m_crColors[ST_BTN_COLOR_FG_IN]);
				pDC->SetBkColor(m_crColors[ST_BTN_COLOR_BK_IN]);
			} // if
			else 
			{
				pDC->SetTextColor(m_crColors[ST_BTN_COLOR_FG_OUT]);
				pDC->SetBkColor(m_crColors[ST_BTN_COLOR_BK_OUT]);
			} // else
			pDC->DrawText(sTitle, -1, captionRect, DT_WORDBREAK | DT_CENTER);
		} // if
	} // if
...
void SG_Button::PaintBk(CDC* pDC)
{
...
HBITMAP SG_Button::CreateBitmapMask(HBITMAP hSourceBitmap, DWORD dwWidth, DWORD dwHeight, COLORREF crTransColor)
{
	HBITMAP		hMask		= NULL;
	HDC			hdcSrc		= NULL;
	HDC			hdcDest		= NULL;
...
//
void SG_Button::PrepareImageRect(BOOL bHasTitle, RECT* rpItem, CRect* rpTitle, BOOL bIsPressed, DWORD dwWidth, DWORD dwHeight, CRect* rpImage)
{
	CRect rBtn;
...
void SG_Button::DrawTheIcon(CDC* pDC, BOOL bHasTitle, RECT* rpItem, CRect* rpTitle, BOOL bIsPressed, BOOL bIsDisabled)
{
	BYTE		byIndex		= 0;
...
void SG_Button::DrawTheBitmap(CDC* pDC, BOOL bHasTitle, RECT* rItem, CRect *rCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
	HDC			hdcBmpMem	= NULL;
...
//
HICON SG_Button::CreateGrayscaleIcon(HICON hIcon)
{
...
//		ST_BTN_INVALIDRESOURCE
//			Failed loading the specified resource.
//
DWORD SG_Button::SetIcon(int nIconIn, int nIconOut)
{
	HICON		hIconIn			= NULL;
	HICON		hIconOut		= NULL;
	...
	if (nIconOut)
	{
		if (nIconOut == (int)ST_BTN_AUTO_GRAY)
			hIconOut = ST_BTN_AUTO_GRAY;
		else
...
//			Failed loading the specified resource.
//
DWORD SG_Button::SetIcon(HICON hIconIn, HICON hIconOut)
{
	BOOL		bRetValue;
	ICONINFO	ii;
...
//
DWORD SG_Button::SetBitmaps(int nBitmapIn, COLORREF crTransColorIn, int nBitmapOut, COLORREF crTransColorOut)
{
...
//			Failed creating mask bitmap.
//
DWORD SG_Button::SetBitmaps(HBITMAP hBitmapIn, COLORREF crTransColorIn, HBITMAP hBitmapOut, COLORREF crTransColorOut)
{
	int		nRetValue;
	BITMAP	csBitmapSize;
...
//				By default, SG_Button buttons are flat.
//		[IN]	bRepaint
...
//			Function executed successfully.
//
DWORD SG_Button::SetFlat(BOOL bFlat, BOOL bRepaint)
{
	m_bIsFlat = bFlat;
...
//				By default, SG_Button buttons have ST_ALIGN_HORIZ alignment.
//		[IN]	bRepaint
//				If TRUE the control will be repainted.
//
// Return value:
...
//
DWORD SG_Button::SetAlign(BYTE byAlign, BOOL bRepaint)
{
	switch (byAlign)
	{    
		case ST_ALIGN_HORIZ:
...
//			Function executed successfully.
//
DWORD SG_Button::SetCheck(int nCheck, BOOL bRepaint)
{
	if (m_bIsCheckBox)
	{
...
//			Function executed successfully.
//
DWORD SG_Button::SetDefaultColors(BOOL bRepaint)
{
	m_crColors[ST_BTN_COLOR_BK_IN]		= ::GetSysColor(COLOR_BTNFACE);
	m_crColors[ST_BTN_COLOR_FG_IN]		= ::GetSysColor(COLOR_BTNTEXT);
	m_crColors[ST_BTN_COLOR_BK_OUT]		= ::GetSysColor(COLOR_BTNFACE);
	m_crColors[ST_BTN_COLOR_FG_OUT]		= ::GetSysColor(COLOR_BTNTEXT);
	m_crColors[ST_BTN_COLOR_BK_FOCUS]	= ::GetSysColor(COLOR_BTNFACE);
	m_crColors[ST_BTN_COLOR_FG_FOCUS]	= ::GetSysColor(COLOR_BTNTEXT);
...
//				ST_BTN_COLOR_BK_IN		Background color when mouse is over the button
//				ST_BTN_COLOR_FG_IN		Text color when mouse is over the button
//				ST_BTN_COLOR_BK_OUT		Background color when mouse is outside the button
//				ST_BTN_COLOR_FG_OUT		Text color when mouse is outside the button
...
//			Invalid color index.
//
DWORD SG_Button::SetColor(BYTE byColorIndex, COLORREF crColor, BOOL bRepaint)
{
	if (byColorIndex >= ST_BTN_MAX_COLORS)	return ST_BTN_INVALIDINDEX;
...
//			Invalid color index.
//
DWORD SG_Button::GetColor(BYTE byColorIndex, COLORREF* crpColor)
{
	if (byColorIndex >= ST_BTN_MAX_COLORS)	return ST_BTN_INVALIDINDEX;
...
//
DWORD SG_Button::OffsetColor(BYTE byColorIndex, short shOffset, BOOL bRepaint)
{
	BYTE	byRed = 0;
	BYTE	byGreen = 0;
	BYTE	byBlue = 0;
	...
	if (byColorIndex >= ST_BTN_MAX_COLORS)	return ST_BTN_INVALIDINDEX;
	if (shOffset < -255 || shOffset > 255)	return ST_BTN_BADPARAM;
...
//			Function executed successfully.
//
DWORD SG_Button::SetAlwaysTrack(BOOL bAlwaysTrack)
{
...
//			Failed loading the specified resource.
//
DWORD SG_Button::SetBtnCursor(int nCursorId, BOOL bRepaint)
{
	HINSTANCE	hInstResource = NULL;
	...
	if (nCursorId)
	{
		...
		if (bRepaint) Invalidate();
		// If something wrong
		if (m_hCursor == NULL) return ST_BTN_INVALIDRESOURCE;
	} // if
...
//			Function executed successfully.
//
DWORD SG_Button::DrawBorder(BOOL bDrawBorder, BOOL bRepaint)
{
...
//			Function executed successfully.
//
DWORD SG_Button::DrawFlatFocus(BOOL bDrawFlatFocus, BOOL bRepaint)
{
...
//
void SG_Button::SetTooltipText(int nText, BOOL bActivate)
{
	CString sText;
...
//				If TRUE the tooltip will be created active.
//
void SG_Button::SetTooltipText(LPCTSTR lpszText, BOOL bActivate)
{
...
//
void SG_Button::ActivateTooltip(BOOL bActivate)
{
...
//				If TRUE the control will be repainted.
//
void SG_Button::DrawTransparent(BOOL bRepaint)
{
...
//			Function executed successfully.
//
DWORD SG_Button::SetURL(LPCTSTR lpszURL)
{
...
//
#ifndef	ST_BTN_USE_BCMENU
DWORD SG_Button::SetMenu(UINT nMenu, HWND hParentWnd, BOOL bRepaint)
{
	HINSTANCE	hInstResource	= NULL;
	...
	if (nMenu)
	{
		...
		// If something wrong
		if (m_hMenu == NULL) return ST_BTN_INVALIDRESOURCE;
	} // if
...
//
#ifdef	ST_BTN_USE_BCMENU
DWORD SG_Button::SetMenu(UINT nMenu, HWND hParentWnd, BOOL bWinXPStyle, UINT nToolbarID, CSize sizeToolbarIcon, COLORREF crToolbarBk, BOOL bRepaint)
{
	BOOL	bRetValue = FALSE;
	...
	if (nMenu)
	{
		...
		// If something wrong
		if (bRetValue == FALSE) return ST_BTN_INVALIDRESOURCE;
...
// This is a virtual function that can be rewritten in SG_Button-derived classes
...
//			Function executed successfully.
//
DWORD SG_Button::OnDrawBackground(CDC* pDC, LPCRECT pRect)
{
	COLORREF	crColor;

	if (m_bMouseOnButton || m_bIsPressed)
		crColor = m_crColors[ST_BTN_COLOR_BK_IN];
	else
	{
		if (m_bIsFocused)
			crColor = m_crColors[ST_BTN_COLOR_BK_FOCUS];
		else
			crColor = m_crColors[ST_BTN_COLOR_BK_OUT];
	} // else
...
//			Function executed successfully.
//
DWORD SG_Button::OnDrawBorder(CDC* pDC, LPCRECT pRect)
{
	if (m_bIsPressed)
